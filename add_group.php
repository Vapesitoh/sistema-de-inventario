...
<?php
  if(isset($_POST['add'])){
   ...
   if(find_by_groupName($_POST['group-name']) === false ){
     $session->msg('d','<b>Lo siento!</b> El nombre de grupo ingresado ya en la base de datos!');
     redirect('add_group.php', false);
   }elseif(find_by_groupLevel($_POST['group-level']) === false) {
     $session->msg('d','<b>Lo siento!</b> El livel de grupo ingresado ya en la base de datos!');
     redirect('add_group.php', false);
   }
   if(empty($errors)){
        ...
        if($db->query($query)){
          ...
          $session->msg('d',' Lo sentimos, no se pudo crear el grupo!');
          redirect('add_group.php', false);
        }
   } else {
...
<div class="login-page">
    <div class="text-center">
       <h3>Agregar nuevo usuario a un grupo</h3>
     </div>
